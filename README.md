...
* **장타 이벤트:** `events ∈ {double, triple, home_run}` 인 투구 (70,068건)
* **메인 — 같은 타자 재대결:** 장타를 맞은 타자의 같은 경기 바로 다음 타석을 같은 투수가 다시 상대한 경우(`next_pa_mode="same_batter"`). 장타 70,068건 중 27,294건(39.0%), 구종 결측을 뺀 27,274건
* **보조 — 다음 타자:** 같은 `game_pk`, 같은 투수, `at_bat_number + 1` 인 타석 — 항상 **다른 타자**이며, 투수 교체 등으로 없으면 `has_next_ab = False`(전체의 6.1%, 분석 제외). 65,815건
...
데이터: 팀 공유 research CSV 표본 — 2021 시즌 ~ 2026-07-09 정규시즌, 투수-시즌 투구 수 500 이상(2026은 300 이상)인 투수 1,064명의 투구 3,579,271건 ([정의](data/research/README.md)). **2026 전반기 표본은 팀에서 확인 중이라 확정되면 수치가 바뀔 수 있습니다.** 이전 표본(전체 수집본 3,993,429건)과 비교하면 순수효과는 약 0.3%p, `group` 오즈비는 0.02 이내로 움직였고 `group × baseline_usage`는 2.30→2.02(재대결), 1.15→1.20(다음 타자)로 이전 신뢰구간 안에서 바뀌었습니다. 부호와 유의성은 그대로입니다(구속 Intensive margin만 비유의인 채 부호가 바뀜).

### 메인: 같은 타자 재대결 (장타 27,274건 vs 대조군 `field_out` 27,274건)

| 단계 | 결과 |
|---|---|
| ① 시즌 전체 구사율 baseline 대비 (대조군 없음) | 사용 비중 29.6% → 18.6%, **−11.0%p** (p≈0) |
| ② 대조군 넷팅 diff-in-diff, 시즌 baseline | 장타 −11.0%p vs 대조군 +3.6%p → **순수 감소 14.6%p** (95% CI [14.1, 15.0], p≈0) |
| ③ 대조군 넷팅, 경기 내 baseline | 장타 −15.7%p vs 대조군 −0.2%p → **순수 감소 15.5%p** (95% CI [15.0, 16.0], p≈0) |
| ④ 재사용률 | 장타 45.5% vs 대조군 67.8% (χ² p≈0) |
| ⑤ 혼합효과 로지스틱 회귀 | `group` 오즈비 **0.285**(평균 baseline에서 약 0.36), `group × baseline_usage` 오즈비 **2.024** (p<0.001) |
...
* 시즌별 감소폭(시즌 baseline, 대조군 없음)은 6개 시즌 모두 약 −10.5%p ~ −11.3%p로 일관됩니다.
  ...
  | 2026 | 2,806 | 26.18% | 15.23% | −10.95%p |

* ⑤ 해석: 장타를 맞으면 재사용 오즈가 대조군보다 크게 낮아지고, 평소 그 구종 의존도가 높을수록 회피 효과가 약해집니다(대체할 구종이 마땅치 않으면 못 피함). 표본 53,623행(장타 26,751 + 대조군 26,872, 경기 내 baseline 결측 제외), 투수 720명, 투수별 기울기 분산 0.0679, 수렴 정상·isSingular 아님.
* 범석님 노트북(Python, 시즌 baseline)과 이 저장소의 독립 재구현은 (전체 수집본 표본에서) 일치했습니다: 재대결 건수는 시즌별 1~7건 차이, 시즌 baseline은 0.02%p 이내, 시즌별 감소폭은 최대 0.10%p 차이. research 표본으로 바꾼 뒤에는 다시 비교하지 않았습니다.
...
**추가 검증 (research 표본 기준으로 재실행, 같은 스크립트와 `run_intensive_margin_analysis.py`)**

| 검증 | 결과 |
|---|---|
| ⑥ 동타/이타(platoon) 혼합모델 | `platoon_match` 오즈비 1.137 [1.073, 1.205] (p<0.001), `group × platoon_match` 오즈비 **0.973** [0.902, 1.049] (p=0.48) — 회피 효과가 동타/이타에 따라 다르다는 증거 없음 |
| ⑥ 순수 감소 스플릿 (시즌 baseline) | 이타 13.9%p vs 동타 15.4%p, 타자 좌/우 14.4%p 대 14.7%p, 투수 좌/우 15.2%p 대 14.3%p. 구종별(표본 300건 이상)은 FF 11.9%p ~ SL 16.9%p |
| ⑦ 투수 단위 매칭 (같은 투수×시즌×구종계열의 `field_out`만 대조군) | 장타의 99.3%(27,100건, 642명)가 1:1 매칭. 순수 감소 14.1%p / 14.9%p로 층화 매칭(14.6 / 15.5)보다 0.4~0.6%p 작지만 같은 방향, `group` 오즈비 0.285 대 0.285, `group × baseline_usage` 2.16 대 2.02(둘 다 p<0.001), 투수별 랜덤 기울기 상관 Pearson 0.997 / Spearman 0.995 |
| ⑨ 구종별 회피 (9개 구종, `run_pitch_type_avoidance_analysis.py`) | 전 구종에서 회피(`group` 오즈비 0.22~0.48, 모두 CI가 1에서 멀음), 구종에 따라 강도가 다름(우도비 검정 χ²=160.8, df=8, p<0.001). FF가 가장 약하고(0.48) CU·CH·FC·SL이 가장 강함(0.22~0.27) |
| ⑩ 구종별 재구사율 감소 (7개 구종 FF·SI·SL·CH·FC·CU·ST, `run_pitch_type_reuse_rate_analysis.py`) | 재대결 구사율이 사전 대비 장타 후 크게 줄고(전체 29.9% → 19.0%), 대조군 보정 순수 감소는 포심 11.9%p ~ 슬라이더 16.9%p(전체 14.5%p), 상대 감소율은 포심 31% ~ 커브 65%(전체 43%). 재사용률은 장타 46.3% vs 대조군 68.3% |
| ⑧ Intensive margin (재사용한 이벤트만: 장타 12,421건, 대조군 18,502건) | 대조군 대비 변화(`group × time`): 코스 **+0.023 ft** [0.013, 0.033] (p<0.0001, 약 0.3인치), 무브먼트 +0.0013 ft (p=0.27), 구속 +0.017 mph [−0.003, 0.036] (p=0.095) |
...
* 코스 변화는 통계적으로는 유의하지만 편차 표준편차(0.52 ft)의 약 4.4%로 실질 크기는 아주 작습니다. 무브먼트·구속에서는 유의한 변화가 없습니다. 다만 구속은 전체 수집본 표본에서 −0.003 mph(p=0.78)였다가 이번에 +0.017 mph(p=0.095)로 부호와 p값이 바뀌었으니, 2026 표본이 확정되면 다시 확인하세요.
* 변수 정의와 결측 처리는 [docs/variable_spec.md](docs/variable_spec.md)에 정리했습니다.

### 보조: 다음 타자 기준 (장타 64,504건 vs 대조군 44,694건)

| 단계 | 결과 |
|---|---|
| ① 경기 내 baseline 대비 (noisy, 대조군 없음) | 사용 비중 35.4% → 25.4%, **−10.0%p** (p≈0) |
| ② 시즌 전체 구사율 baseline 대비 | 31.7% → 25.6%, **−6.1%p** (p≈0) — 평균회귀 일부 확인 |
| ③ 대조군(field_out) 넷팅 diff-in-diff | 장타 −10.0%p vs 대조군 −2.5%p → **순수 감소 7.57%p** (95% CI [7.20, 7.94], p≈0), ①의 75.4% |
| ④ 혼합효과 로지스틱 회귀 | `group` 오즈비 **0.575** (p≈4e-92), `group × baseline_usage` 오즈비 **1.202** (p=0.007) |
| ⑤ robustness (랜덤효과 구조 단순화) | 원본 공식(`(1 + group \| pitcher)`, `catcher_changed` 포함)은 이번에도 isSingular=True(절편-기울기 상관 −1.0, 기울기 분산 0.0008). 단순화 모델은 singular가 아니고(기울기 분산 0.0568) `group` 오즈비도 방향·유의성이 같아(원본 0.535 대 0.575) 메인 공식으로 유지. 다만 두 모델의 투수별 기울기 상관은 Pearson −0.855로, 전체 수집본에서 1.000이던 것과 달라졌습니다. 원본은 퇴화해 투수별 기울기를 해석할 수 없으므로 투수별 순위는 단순화 모델로만 보세요 |

* 모델: `reused_same_type ~ group * baseline_usage + balls + strikes + outs_when_up + score_diff + stand + pitch_family + factor(season) + (0 + group | pitcher)`, R `lme4::glmer`(nAGQ=0, bobyqa). 표본 109,198행, 투수 1,064명. 예측확률 10분위 calibration은 전 구간에서 근접.
...
* 참고로 경기 내 baseline 기준 순수 감소는 재대결 15.5%p, 다음 타자 7.57%p이고 혼합모델 `group` 오즈비는 0.285 대 0.575입니다. 다만 재대결은 타자가 다시 타석에 설 때까지 투수가 남아 있는 경우로 한정돼 모집단(투수 유형, 경기 시점 등)이 다르므로, 이 차이가 "그 타자에 대한 맞춤 조정" 때문인지는 아직 분리되지 않았습니다.
...
* `data/research/`: 팀 공유 분석 표본 CSV (git 제외, [설명](data/research/README.md)). 분석은 `data/raw`에서 이 CSV에 있는 행만 골라 씁니다
...
- [x] 분석 표본을 팀 공유 research CSV로 통일하고 전체 분석·문서 재실행 (2026-09-25, `Dongyun` 브랜치)
- [ ] 2026 전반기 표본(07-09까지, 컷오프 300구) 확정 — 팀 확인 중
...
* 메인 분석의 대조군은 시즌 baseline에서 오히려 사용 비중이 늘어납니다(+3.6%p). "아웃이라는 성공 때문에 다시 던진다"(성공 강화)가 아니라 "그 구종이 이 경기에서 이미 던져졌다"는 조건 자체 때문입니다. 결과와 무관한 첫 투구를 앵커로 한 표본도 +3.6%p, 삼진 +3.7%p, 볼넷 +2.4%p로 같은 양상이고, 단타 −3.7%p, 장타 −11.0%p로 결과가 나쁠수록 감소가 커집니다(`run_outcome_anchor_check.py`). 이 효과는 장타 그룹에도 똑같이 작용하고, `field_out` 대조군은 결과 무관 앵커와 사실상 같아(차이 −0.01%p) 대조군으로 걷어내는 게 맞습니다.
...
* 분석 표본은 시즌 투구 수 500구 이상(2026은 300구 이상) 투수로 한정돼 투구량이 적은 투수(구원·신인 등)가 빠집니다. 전체 수집본과 비교하면 투수는 1,858명에서 1,064명으로 줄지만 투구는 약 10%만 줄고, 제외된 투수-시즌만 따로 본 순수효과(재대결 15.4%p, 다음 타자 7.3%p)는 포함된 쪽(14.2%p, 6.8%p)과 신뢰구간이 겹쳐 결론은 같았습니다. 다만 제외 투수의 구성(선발/구원 등)은 따로 분석하지 않았고, 2026은 시즌 일부만 반영된 표본이라 확정 전입니다.
* `catcher_changed`(포수 교체 여부)는 모델에서 제외했습니다. `has_next_ab=True`는 하프이닝이 끊기지 않은 구간이라 포수 교체가 구조적으로 거의 없어(109,198건 중 4건) 계수를 추정할 수 없습니다. 컬럼 자체는 이벤트 데이터셋에 남아 있습니다.
