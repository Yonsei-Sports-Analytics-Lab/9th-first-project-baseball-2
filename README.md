...
데이터: 팀 공유 research CSV 표본 — 2021 시즌 ~ 2026-07-12 정규시즌, 투수-시즌 투구 수 500 이상(2026은 300 이상)인 투수 1,067명의 투구 3,592,302건 ([정의](data/research/README.md)). 2026 파일은 수정본(2026-09-25 반영)입니다. 이전 표본(전체 수집본 3,993,429건)과 비교하면 순수효과는 약 0.3%p 커졌고(재대결 시즌 baseline 14.3→14.6%p, 다음 타자 경기 내 baseline 7.25→7.58%p), `group` 오즈비는 재대결 0.278→0.286, 다음 타자 0.592→0.569, `group × baseline_usage`는 재대결 2.30→1.99, 다음 타자 1.15→1.21로 움직였습니다. 새 추정치는 모두 전체 수집본 95% 신뢰구간 안에 있고 부호와 유의성은 그대로입니다. 바뀐 것은 부차적인 진단 두 가지입니다: 구속 Intensive margin의 p값이 표본마다 흔들리고(⑧), 다음 타자 원본 공식의 singular 여부가 바뀌었습니다(⑤).
...
* 코스 변화는 통계적으로는 유의하지만 편차 표준편차(0.52 ft)의 약 3.9%로 실질 크기는 아주 작습니다. 무브먼트에서는 유의한 변화가 없습니다. 구속은 이번에 p=0.048로 5% 안쪽에 들어왔지만 신뢰구간 하한이 0.0002 mph로 0에 붙어 있고 효과 크기(+0.020 mph)는 편차 표준편차(1.25 mph)의 약 1.6%입니다. 표본이 조금 바뀔 때마다 결과가 움직였습니다(전체 수집본 −0.003 mph, p=0.78 → 2026 수정 전 CSV +0.017 mph, p=0.095 → 이번 +0.020 mph, p=0.048). 세 지표를 함께 검정하는 점까지 고려하면 "구속이 바뀐다"는 근거로 쓰기 어렵고, 코스·무브먼트·구속 모두 실질적 변화는 작다고 보는 편이 안전합니다.
...
| ⑤ robustness (랜덤효과 구조 단순화) | 원본 공식(`(1 + group \| pitcher)`, `catcher_changed` 포함)은 이번 표본에서 isSingular=False로 나왔지만(2026 수정 전 CSV 표본과 전체 수집본에서는 True) 기울기 분산이 0.0007로 사실상 0이고 절편-기울기 상관은 −0.63입니다. `catcher_changed`(4건)는 추정할 수 없습니다(오즈비 0.37, p=0.37). 단순화 모델은 singular가 아니고(기울기 분산 0.0569) `group` 오즈비도 방향·유의성이 같아(원본 0.531 대 0.569) 메인 공식으로 유지. 다만 두 모델의 투수별 기울기 상관은 Pearson −0.82 / Spearman −0.80으로, 전체 수집본에서 1.000이던 것과 정반대 방향입니다. 원본은 기울기 분산이 거의 0이라 투수별 기울기에 담긴 정보가 거의 없으므로 투수별 순위는 단순화 모델로만 보세요 |
...
- [x] 2026 표본 수정본(07-12까지, 컷오프 300구, 391,038행) 반영, 전체 분석·문서 재실행 (2026-09-25, `Dongyun` 브랜치)
...
* 분석 표본은 시즌 투구 수 500구 이상(2026은 300구 이상) 투수로 한정돼 투구량이 적은 투수(구원·신인 등)가 빠집니다. 전체 수집본과 비교하면 투수는 1,858명에서 1,067명으로 줄지만 투구는 약 10%만 줄고, 제외된 투수-시즌만 따로 본 순수효과(시즌 baseline, 재대결 15.3%p [13.1, 17.5], 다음 타자 7.3%p [6.3, 8.3])는 포함된 쪽(14.2%p [13.8, 14.7], 6.8%p [6.4, 7.1])과 신뢰구간이 겹쳐 결론은 같았습니다. 다만 제외 투수의 구성(선발/구원 등)은 따로 분석하지 않았고, 2026은 07-12까지만 반영된 표본입니다.
