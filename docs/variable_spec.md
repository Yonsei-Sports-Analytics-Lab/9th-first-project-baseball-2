...
|---|---|---|---|
| `pitch_reuse_after_xbh_events.parquet` (`.csv`) | 70,319 | 장타 1건 (비교 타석 = 다음 타자) | `src/preprocessing/data_pipeline.py` |
| `pitch_reuse_after_xbh_same_batter_rematch_events.parquet` | 27,391 | 장타 중 같은 타자·같은 투수 재대결이 있는 1건 | `run_same_batter_rematch_analysis.py` |
| `pitch_reuse_placebo_same_batter_rematch_events.parquet` | 27,371 | 대조군 `field_out` 재대결 1건 (시즌×구종계열 층화) | 〃 |
| `pitch_reuse_placebo_same_batter_rematch_pitcher_matched_events.parquet` | 27,197 | 대조군 재대결 1건 (같은 투수×시즌×구종계열) | 〃 |
| `intensive_margin_pitch_observations.parquet` | 362,630 | 투구 1개 (pre/post 관측) | `run_intensive_margin_analysis.py` |
...
| `baseline_usage` | 그 경기에서 이벤트 투구 **이전**까지 같은 투수의 `hit_pitch_type` 비중 | 이전 투구가 없거나 구종이 없으면 NaN (1.99% / 1.51%) |
...
* `catcher_changed`는 다음 타자 기준에서는 모델 표본 109,536건 중 4건만 1이라 모델에서 뺐습니다. 재대결에서는 장타 122건(0.4%), 대조군 100건(0.4%)로 늘어나지만 여전히 드뭅니다. 이 변수는 아직 어떤 모델에도 넣지 않았습니다.
...
**포함되는 이벤트:** `has_next_ab`이고 `reused_same_type == 1`인 이벤트(장타 12,459건, 대조군 18,579건). 재사용하지 않은 이벤트는 비교할 투구가 없어 제외합니다.
