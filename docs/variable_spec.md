...
2026-09-24 기준, 모든 수치는 현재 `data/processed/` 파일에서 직접 센 값입니다. 파일은 `.gitignore` 대상이라 각자 아래 "생성 코드"로 다시 만들어야 합니다.

## 1. 데이터 파일

| 파일 (`data/processed/`) | 행 수 | 1행의 의미 | 생성 코드 |
|---|---|---|---|
| `pitch_reuse_after_xbh_events.parquet` (`.csv`) | 79,285 | 장타 1건 (비교 타석 = 다음 타자) | `src/preprocessing/data_pipeline.py` |
| `pitch_reuse_after_xbh_same_batter_rematch_events.parquet` | 28,765 | 장타 중 같은 타자·같은 투수 재대결이 있는 1건 | `run_same_batter_rematch_analysis.py` |
| `pitch_reuse_placebo_same_batter_rematch_events.parquet` | 28,745 | 대조군 `field_out` 재대결 1건 (시즌×구종계열 층화) | 〃 |
| `pitch_reuse_placebo_same_batter_rematch_pitcher_matched_events.parquet` | 28,328 | 대조군 재대결 1건 (같은 투수×시즌×구종계열) | 〃 |
| `intensive_margin_pitch_observations.parquet` | 375,700 | 투구 1개 (pre/post 관측) | `run_intensive_margin_analysis.py` |
...
| `baseline_usage` | 그 경기에서 이벤트 투구 **이전**까지 같은 투수의 `hit_pitch_type` 비중 | 이전 투구가 없거나 구종이 없으면 NaN (2.05% / 1.69%) |
...
* `catcher_changed`는 다음 타자 기준에서는 123,044건 중 5건만 1이라 모델에서 뺐습니다. 재대결에서는 장타 154건(0.5%), 대조군 143건(0.5%)로 늘어나지만 여전히 드뭅니다. 이 변수는 아직 어떤 모델에도 넣지 않았습니다.
* 구종이 없는 이벤트 투구(장타 39건)는 재사용 관련 값이 모두 NaN입니다. 0으로 채우지 않습니다.
...
**포함되는 이벤트:** `has_next_ab`이고 `reused_same_type == 1`인 이벤트(장타 13,103건, 대조군 19,400건). 재사용하지 않은 이벤트는 비교할 투구가 없어 제외합니다.

* **pre (`time=0`):** 같은 경기에서 그 투수가 던진 같은 구종 투구 중 이벤트 투구 **이전**의 것 (이벤트 투구 자체는 제외). 이벤트당 중앙값 8개
...
| `pfx_x`, `pfx_z` | 무브먼트 (ft) | 0.02% 미만 |
| `release_speed` | 구속 (mph) | 0.01% 미만 |
| `baseline_usage`, `platoon_match` | 이벤트의 값을 복사 | `baseline_usage` 0.34% |
| `base_plate_x/z`, `base_pfx_x/z`, `base_release_speed` | 그 투수·시즌·구종의 평소 값(평균) | 표본 부족 시 NaN (0.4% / 0.06%) |
| `location_dev` | `sqrt((plate_x − base_plate_x)² + (plate_z − base_plate_z)²)`, ft | baseline이 없거나 위치가 없으면 NaN (0.41%) |
| `movement_dev` | `sqrt((pfx_x − base_pfx_x)² + (pfx_z − base_pfx_z)²)`, ft | NaN (0.07%) |
| `velocity_dev` | `release_speed − base_release_speed`, mph (부호 있음) | NaN (0.06%) |
...
* 편차 분포: `location_dev` 평균 0.94 ft(최대 6.5), `movement_dev` 평균 0.22 ft(최대 3.3), `velocity_dev` 평균 0.10 mph(−26 ~ +16). 이상치는 그대로 두었습니다.
